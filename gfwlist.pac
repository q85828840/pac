// 基于gfwlist自动生成的PAC脚本
// 生成时间: 2025-09-01 19:14:21
function FindProxyForURL(url, host) {
    var proxy = "PROXY 192.168.3.252:7890; SOCKS5 192.168.3.252:7891";
    var direct = "DIRECT";
    
    var checkHosts = [        "connectivitycheck.huawei.com,
        "hwcloudtest-drcn.dbankcloud.cn,
        "connectivitycheck.gstatic.com,
        "captive.apple.com"];
    for (var i = 0; i < checkHosts.length; i++) {
        if (host === checkHosts[i] || host.endsWith("." + checkHosts[i])) {
            return direct;
        }
    }
    ...
    if (isPlainHostName(host) || 
        ...
        isInNet(ipv4(host), "172.16.0.0", "255.240.0.0") ||
        isInNet(ipv4(host), "192.168.0.0", "255.255.0.0") ||
        isInNet(ipv4(host), "127.0.0.0", "255.255.255.0")) {
        return direct;
    }
    
    var rules = [    "[AutoProxy 0.2.9],
    "||blogjav.net,
    "||zoominfo.com,
    "||ptwxz.com,
    "||miuipolska.pl,
...
function matchRule(host, url, rule) {
    if (!rule || rule[0] === '!' || rule.trim() === '') return false;
    ...
    if (rule.includes("*")) {
        var pattern = rule.replace(/\*/g, ".*").replace(/\./g, "\\.");
        return new RegExp("^" + pattern + "$").test(host) || 
               new RegExp("^" + pattern).test(url);
    }
    
    return host === rule || url.includes(rule);
}

function ipv4(host) {
    try { return dnsResolve(host); } catch (e) { return host; }
}
